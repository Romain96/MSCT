...
class MultiScaleComponentTree():
	"""
	This class represents a multi-scale component-tree (MSCT).
	...
	def __init__(self):
		"""
		Constructor - initializes **root** to *None* and **nodes** to an empty set, also creates a pixel converter.
	...
	def get_root(self) -> MultiScaleComponentTreeNode:
		...
		: MultiScaleComponentTreeNode
			The tree's root node (can be *None*).
	...
	def get_nodes(self) -> set[MultiScaleComponentTreeNode]:
		...
		Returns
		-------
		: set[MultiScaleComponentTreeNode]
			The set of all the tree's nodes (including the root).
	...
	def set_nodes(self, nodes: set[MultiScaleComponentTreeNode]) -> None:
		...
		Parameters
		----------
		nodes: set[MultiScaleComponentTreeNode]
			The set of all the tree's nodes (including the root).
	...
	def get_invert(self) -> bool:
		...
		-------
		: bool
			*True* if the image has been inverted, *False* otherwise.
	...
	def get_pconverter(self) -> PixelPower2ScaleConverter:
		...
		: PixelPower2ScaleConverter
			An instance of the PixelPower2ScaleConverter used to convert pixels between 1D indices and 2D (line, column) pixels.
	...
	def set_pconverter(self, pconverter: PixelPower2ScaleConverter) -> None:
		...
		Parameters
		----------
		pconverter: PixelPower2ScaleConverter
			An instance of the PixelPower2ScaleConverter used to convert pixels between 1D indices and 2D (line, column) pixels.

		Note
		----
		**pconveter** should be initialized with the same width as the image used to build the MSCT.
	...
	def get_node_by_id(self, id: int) -> MultiScaleComponentTreeNode:
		"""
		Retrieves a node given its unique ID.
		...
		-------
		: MultiScaleComponentTreeNode
			The node whose unique ID is **id** or *None* if not found.
	...
	def init_from_component_tree(self, tree: ComponentTree, pixel_converter: Index2Pixel, scale: int) -> None:
		"""
		Initializes a multi-scale component-tree from a component-tree at a lower scale (**n** successive subsampling).

		Parameters
		----------
		tree: ComponentTree
		 	A component-tree build on a downsampled image.
		pixel_converter: Index2Pixel
			An index to pixel converter.
		scale: int
			The scale at which the pixels are defined.
	...
	def compute_height(self, node: MultiScaleComponentTreeNode, height: int) -> int:
		"""
		Computes the height of the local branch starting at the given node.

		Parameters
		----------
		node: MultiScaleComponentTreeNode
			Root node for the height computation.
		height: int
			The current height at the node.

		Returns
		-------
		: int
			The total height of the deepest branch i.e. the height at the current node + the height of its deepest sub branch.
	...
	def compute_subarea(self, node: MultiScaleComponentTreeNode) -> int:
		...
		----------
		node: MultiScaleComponentTreeNode
			The root node at which to start the computation.

		Returns
		-------
		: int
			The accumulated subarea at the node.
	...
	def get_border_pixels(self, node: MultiScaleComponentTreeNode, scale=0) -> tuple[set]:
		...
		----------
		node: MultiScaleComponentTreeNode
			The node representing the flat zone.
		scale: int
			The scale at which the computation tales place (global scale by default).

		Returns
		-------
		: tuple[set]
			The set of pixels belonging to the *boundary* and set of pixels belonging to the *interior*.
	...
	def compute_mser(self, node: MultiScaleComponentTreeNode, delta: int) -> None:
		...
		----------
		node: MultiScaleComponentTreeNode
			The MSCT node on which to compute MSER stability.
		delta: int
			The level delta upward and downward in the tree.

		Returns
		-------
		None, assigns the **mser**, **area_derivative_delta_level** and **area_derivative_delta_area** attributes of MSCT node.
	...
	def compute_mser_alt(self, node: MultiScaleComponentTreeNode, delta: int) -> None:
		"""
		Alternative computation of MSER stability values.

		Parameters
		----------
		node: MultiScaleComponentTreeNode
			The to for which the MSER value is computed.
		delta: int
			The delta parameter for MSER.
	...
	def upward(self, node: MultiScaleComponentTreeNode, delta: int, level: int) -> MultiScaleComponentTreeNode:
		"""
		Computes the MSER stability value upwards.
		...
		delta: int
			The delta parameter for MSER.
		level: int
			the current gray level value.

		Returns
		-------
		: MultiScaleComponentTreeNode
			The node at gray level **level** - **delta**.
	...
	def downward(self, node: MultiScaleComponentTreeNode, delta: int, level: int) -> set[MultiScaleComponentTreeNode]:
		"""
		Computes the MSER stability value downwards.

		Parameters
		----------
		node: MultiScaleComponentTreeNode
			The currenly processed node.
		delta: int
			The delta parameter for MSER.
		level: int
			The current gray level value.
		...
		-------
		set[MultiScaleComponentTreeNode]
			The set of all nodes at gray level **level** + **delta**.
	...
	def augment_node_sequential(
		...
		"""
		Augments a node using a double scale resolution image (intended to be used sequentially).
		...
		node: MultiScaleComponentTreeNode
			The node to augment.
		target_scale: int
			The scale of the new nodes to be created.
		scale_image: ndarray
			The image at double scale (as Numpy array).
		pixel_converter: Index2Pixel
			An index <-> pixel converter.
		invert_image: bool
			*True* if the image has to be inverted before computation, *False* otherwise.

		Returns
		-------
		: MultiScaleComponentTreeNode
			The new local root node.
	...
	def build_partial_max_trees_from_flatzone(
		...
		"""
		Builds a set of partial max-trees on a given flatzone.

		Parameters
		----------
		node: MultiScaleComponentTreeNode
			The node on which to build the partial max-trees.
		image: ndarray
			The image (as Numpy array).
		pixel_converter: Index2Pixel
			An instance of an Index2Pixel.
		invert_image: bool
			*True* if the image has to be inverted before computation, *False* otherwise.

		Returns
		-------
		: set[MultiScaleComponentTree]
			The set of all partiel max-trees as MSCTs.
	...
	def define_replace_trees(self, partial_max_tree: Self, level: int) -> set[MultiScaleComponentTreeNode]:
		...
		----------
		partial_max_tree: MultiScaleComponentTree
			The computed partial max-tree.
		level: int
			Gray-level of the root node upon which the partial max-tree has been computed.
	...
	def find_replace_roots(self, node: ComponentTreeNode, level: int, roots: set[ComponentTreeNode]) -> None:
		"""
		Finds the nodes which have to be replaced by the newly computed partial max-trees.

		Parameters
		----------
		node: ComponentTreeNode
			the currently evaluated node.
		level: int
			The current gray level.
		roots: set[ComponentTreeNode]
			The set of all local nodes to be replaced.
	...
	def merge_pmt_with_msct(
		...
		Parameters
		----------
		partial_max_trees: set[MultiScaleComponentTree]
			The set of partial max-trees computed from the original node **replace_node**.
		insert_node: MultiScaleComponentTreeNode
			The node at which the replacement subtrees will be inserted (father of **replace_node**).
		replace_node: MultiScaleComponentTreeNode
			The node on which the partial max-trees are computed as replacement.

		Returns
		-------
		None, updates the MSCT by removing nodes and adding new ones.
	...
	def remove_subtree(self, start: MultiScaleComponentTreeNode) -> None:
		...
		-------
		: None
			Removes nodes from the **nodes** attribute.
	...
	def copy_subtree(self, start: MultiScaleComponentTreeNode) -> tuple[list[MultiScaleComponentTreeNode], dict]:
		...
		----------
		start: MultiScaleComponentTreeNode
			The root of the subtree to copy.

		Returns
		-------
		nodes: list
			A list of copies of nodes of the subtree rooted in **start**.
		copied_ids: dict
			A dictionnary of mappings from original nodes and their copies.
	...
	def insert_subtree(
		...
		----------
		insert_node: MultiScaleComponentTreeNode
			The node of the MSCT after which the subtree will be inserted.
		copied_nodes: set[MultiScaleComponentTreeNode]
			The set of nodes to add to the MSCT.

		Returns
		-------
		: None
			Inserts new nodes into the **nodes** attribute.
	...
	def get_pmt_upward_branch(
		...
		Parameters
		----------
		partial_max_tree: MultiScaleComponentTreeNode
			The partial max-tree on which the branch is extracted.
		start: MultiScaleComponentTreeNode
			The root node at which the branch starts.

		Returns
		-------
		nodes: list[MultiScaleComponentTreeNode]
			The list of ancestor nodes from **start** to the root of **partial_max_tree**.
	...
	def get_subtree_ids(self, root: MultiScaleComponentTreeNode) -> set[int]:
		...
		----------
		root: MultiScaleComponentTreeNode
			The root node from which to start gathering IDs.

		Returns
		-------
		ids: set[int]
			The set of node IDs for the subtree rooted in **root**.
	...
	def decide_subtree_insertion(self, processed_ids: set[int], root: MultiScaleComponentTreeNode) -> bool:
		...
		processed_ids: set[int]
			The set of node IDs of already inserted/merged partial max-tree nodes in the MSCT.
		root: MultiScaleComponentTreeNode)
			The root of the partial max-tree subtree to insert.

		Returns
		-------
		insert: bool)
			*True* if the subtree is to be inserted, *False* otherwise.
	...
	def build_node_content_image(self, image: np.ndarray, pixels: set[tuple[int, int]]) -> np.ndarray:
		...
		----------
		image: ndarray
			A reference image (same size as the original image used to build the MSCT).
		...
		-------
		zone: ndarray
			The image with **pixels** as foreground.
	...
	def gather_subtree_nodes(self, node: MultiScaleComponentTreeNode) -> set[MultiScaleComponentTreeNode]:
		...
		Returns
		-------
		processed: set[MultiScaleComponentTreeNode]
			A set containing all nodes in the subtree of **node**.
	...
	def extract_mask_and_mask_image(self, scale_image: np.ndarray, indices: set[int]) -> np.ndarray:
		...
		----------
		scale_image: ndarray
			A reference image (same size as the image used to build the MSCT).
		indices: set[int]
			A set of pixel indices (flatten in 1D).
				
		Returns
		-------
		mask: ndarray
			The binary mask of the flatzone.
		mask_image: ndarray
			The grayscale image of the flatzone.
	...
	def get_local_flat_zone(self, node: MultiScaleComponentTreeNode, scale: int) -> set[tuple[int, int]]:
		...
		node: MultiScaleComponentTreeNode
			The root node.
		scale: int
			The target scale to express the flat zone.

		Returns
		-------
		pixels: set[tuple[int, int]]
			The set of pixels forming the flat zone of node.
	...
	def build_channel_histogram(self, channel_images: list[np.ndarray]) -> None:
		...
		----------
		channel_images: list
			The list of channel images except the base channel used to build the MSCT.
	...
	def rec_build_channel_histogram(self, node: MultiScaleComponentTreeNode, channel_images: list[np.ndarray]) -> list[int]:
		...
		----------
		node: MultiScaleComponentTreeNode
			The node upon which the histogram is computed.
		channel_images: list
			The list of channel images except the base channel image.

		Returns
		-------
		hist: list
			The locally computed histogram for **node**.
	...
	def save_dot(self, filename: str, mode='dot') -> None:
		...
		mode: str
			'png' or 'dot' or 'svg' whether to save the tree as a PNG file, a DOT file or a SVG file.
	...
	def save_dot_highlight(self, filename: str, nodes: set[MultiScaleComponentTreeNode], mode='dot') -> None:
		"""
		Saves the MSCT in the DOT language in a dot file usinh and PyDot/GraphViz.
	...
	def save_dot_highlight_from_node(self, filename: str, node: MultiScaleComponentTreeNode, nodes: set[MultiScaleComponentTreeNode], mode='dot') -> None:
		"""
		Saves the MSCT in the DOT language in a dot file usinh and PyDot/GraphViz.
		...
		nodes: set
			The set of nodes to highlight.
		mode: str
			'png' or 'dot' or 'both' whether to save the tree as a PNG file, a DOT file or a SVG file.
	...
	def reconstruct_subtree(self, root: MultiScaleComponentTreeNode, image: np.ndarray) -> np.ndarray:
		...
		----------
		node: MultiScaleComponentTreeNode
			The node for which to construct the flat zone.
		image: ndarray
			The Numpy array of the original image

		Returns
		-------
		reconstructed: ndarray
			The reconstructed grayscale subtree image.
	...
	def reconstruct_flat_zone(self, node: MultiScaleComponentTreeNode, image: np.ndarray, binary=False) -> np.ndarray:
		...
		image: ndarray
			The Numpy array of the original image.
		binary: bool
			If *True* the uses 255 for the FZ, otherwise use the node's (gray) level.

		Returns
		-------
		reconstructed: ndarray
			The reconstructed grayscale flat zone image.
	...
	def get_node_content(self, node: MultiScaleComponentTreeNode) -> set:
		...
		----------
		root : MultiScaleComponentTreeNode
			The node for which the flat zone has to be fetched.

		Returns
		-------
		all_pixels: set
			The set of pixels of the flat zone.
	...
	def reconstruct_image(self, image: np.ndarray) -> np.ndarray:
		...
		-------
		reconstructed: ndarray
			The Numpy array of the reconstructed image.
	...
	def get_active_ascendant_level(self, node: MultiScaleComponentTreeNode) -> int:
		...
		Returns
		: int
			The gray level of the node being the closest ascendent which is active.
	...
	def export_tree_as_csv(self, output_dir: str, output_name: str) -> None:
		"""
		...
		output_name: str
			The CSV filename without the extension.
