...
class MSCTMultiChannelWrapper():
	"""
	This class provides a wrapper for the construction and storage of a MSCT on a multi-layer image.
	...
	def __init__(self):
		"""
		Initializes a multi-channel wrapper for the MSCT.
	...
	def reset(self):
		"""
		Resets a multi-channel wrapper for the MSCT.
	...
	def get_base_channel(self) -> int:
		...
		-------
		: int
			The index of the base channel image upon which the MSCT is constructed.
	...
	def get_base_channel_images(self) -> list[np.ndarray]:
		...
		-------
		: list[ndarray]
			The list of base channel images as Numpy arrays.
	...
	def set_base_channel_images(self, base_channel_images: list[np.ndarray]) -> None:
		...
		Parameters
		----------
		base_channel_images : list[ndarray]
			The list of base channel images as Numpy arrays.
	...
	def get_base_channel_image_at_scale(self, scale: int) -> np.ndarray:
		...
		----------
		scale: int
			The scale at which the image should be retrieved (0 = original scale).

		Returns
		-------
		image: ndarray
			The stored image at the **scale** scale
	...
	def get_channel_images(self) -> list[np.ndarray]:
		...
		-------
		: list[ndarray]
			The list of channel images as Numpy arrays.
	...
	def get_channel_image_at_index(self, channel: int, scale: int) -> np.ndarray:
		...
		----------
		channel: int
			The index at which the image should be retrieved.

		Returns
		-------
		image: ndarray
			The stored image of the **channel**-th image.
	...
	def get_annotations(self) -> dict:
		...
		-------
		: dict
			The dictionary of all image annotations.
	...
	def get_pixel_converter(self) -> Index2Pixel:
		...
		-------
		: Index2Pixel
			The pixel converter used for pixel/scale management.
	...
	def get_scale_converter(self) -> PixelPower2ScaleConverter:
		...
		: PixelPower2ScaleConverter
			The scale converter used for pixel/scale management
	...
	def get_added_rows(self) -> int:
		...
		-------
		: int
			The number of added rows to the original image (>= 0).
	...
	def get_added_columns(self) -> int:
		...
		-------
		: int
			The number of added columns to the original image (>= 0).
	...
	def apply_median_filter(self, image: np.ndarray, size: int) -> np.ndarray:
		"""
		Applies a median filter of size **size**x**size** to the layer image defined by **image**.

		Parameters
		----------
		image: ndarray
			The grayscale layer image.
		size: int
			The median filter size (> 0).
				
		Returns
		-------
		filtered: ndarray
			The median filtered grayscale image.
	...
	def fix_image_dimensions_for_subdivision(self, image: np.ndarray, n: int) -> np.ndarray:
		...
		-------
		fixed: ndarray
			Resulting image, same as **image** but with added empty rows/columns.
	...
	def apply_CLAHE(self, image: np.ndarray) -> np.ndarray:
		...
		-------
		enhanced: ndarray
			The resulting grayscale image after CLAHE is applied.
	...
	def downsample_image(
		...
		Parameters
		----------
		channel_images: ndarray
			The list of channel images as Numpy arrays.
		base_channel: int
			The index of the base channel upon which the MSCT will be computed.
		n: int
			The number of successive downsampling steps.
		method: str
			The downsampling method (choices are 'maximum', 'minimum' or 'mean').
		use_median_filter: bool
			Whether to apply a median filter to the image before processing it.
		median_filter_size (int)
			The size of the median filter, only relevant if use_median_filter is set to True.
		invert_image: bool
			Whether to invert image values (black on white instead as white on black).
		normalize: bool
			Whether to normalize images or load them as is.
	...
	def build_base_msct(self, inverted_image=False) -> MultiScaleComponentTree:
		"""
		Builds a MSCT on the lowest scale image.
	...
	def compute_mser(self, delta: int) -> None:
		...
		----------
		delta: int
			The delta parameter on gray levels for the MSER computation.
	...
	def load_miedge_annotations(self, filepath: str) -> None:
		...
		----------
		filepath: str
			The path to the CSV file containing dot annotations.

		Returns
		-------
		: None, assigns the annotations attribute.
	...
	def divide_objects(
		...
		max_area_factor: float
			The maximum area for chosen nodes.
		min_area: int
			The minimum area to consider a flatzone as a nucleus.

		Returns
		-------
		all_objects: list[ndarray]
			The list of segmented nuclei (each a Numpy array).
		channel_vectors: list[list[int]]
			The list of channel vectors for each nucleus (each a list of int).
	...
	def export_objects_as_images(self, images: list[np.ndarray], output_dir: str, all: bool) -> None:
		...
		----------
		images: list[ndarray]
			The list of clusters (each being a boolean image in the form of a Numpy array).
		output_dir: str
			The output directory path.
		all: bool
			Whether to save all individual objects as images or just a combined colour image.
	...
	def export_objects_as_label_image(self, images: list[np.ndarray], output_dir: str) -> None:
		...
		----------
		images: list[ndarray]
			The list of binary segmented nuclei (as Numpy arrays).
		output_dir: str
			The output directory path.
	...
	def export_channel_vectors_as_array(self, vectors: list[list[int]], output_dir: str) -> None:
		...
		----------
		channel_vectors: list[list[int]]
			The list of channel vectors for each nucleus (each being a list of int).
		output_dir: str
			The output directory path.
	...
	def save_subsampled_images(self, subsampled_dir: str, subsampled_name: str) -> None:
		...
		----------
		subsampled_dir: str
			The output directory for saving subsampled images.
		subsampled_name: str
			The subsampled image name prefix.
	...
	def reconstruct_image(self, reconstructed_dir: str, reconstructed_name: str, normalize: bool) -> None:
		...
		reconstructed_name: str
			The reconstructed image name prefix.
		normalize: bool
	...
	def normalize_image(self, image: np.ndarray):
		...
		----------
		image: ndarray
			The input grayscale image as Numpy array.
