...
def process_image_msct(
	...
	"""
	Builds a multi-scale component-tree (MSCT) on an image using a given number of downsampling and enrichment scales.
	...
	subsample: int
		The number of downsampling steps (scales) to use.
	enrich_area_factor: float
		The percentage of the total image area allowed fror enrighment process [0,1]
	enrich_max_mser: float
		The maximum MSER value allowed for a node to be selected during the node selection step.
	mser_percent_height: float
		The percentage of the MSCT height to use for MSER computation [0,1]
	segmentation_max_mser: float
		The maximum MSER value allowed for a node during the cell segmentation step.
	segmentation_min_area: int
		The minimum surface area of final clusters.
	...
	aug_fz_dir = os.path.join(output_dir, 'augmented_flatzones')
	...
	create_dir_remove_if_exist(aug_fz_dir)
	...
	if is_colour:
		wrapper.subsample_image(
			image=gs_image, n=subsample, method='maximum', 
...
if __name__ == '__main__':
	...
	ap.add_argument('--enrich_area_factor', help='Maximum flat zone area to be enriched in percent of image size [0,1]', default=0.1)
	...
	ap.add_argument('--segmentation_min_area', help='Maximum surface area for the segentation step', default=16)
