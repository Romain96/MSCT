...
def build_node_content_image(image: np.ndarray, pixels: set) -> np.ndarray:
	...
	----------
	image: ndarray
		Image of the same dimensions as the result image to create.
	pixels: set
		Set of pixels (line, column) to assign to the foreground.

	Returns
	-------
	zone: ndarray
		Result binary image of the same dimension as **image**.
...
def get_images_in_dir(directory: str, extension='png') -> list[str]:
	"""
...
def load_gt_masks(images: list[np.ndarray]) -> list[np.ndarray]:
	"""
	Loads all ground truth masks images.
...
def load_pred_masks(images: list[str]) -> list[np.ndarray]:
	"""
	Loads all predicted masks images.

	Parameters
	----------
	images: list[str]
		Path to the images as a list.

	Returns
	-------
	all_images: list[ndarray]
		List of loaded images.
...
def compute_intersection_over_union(gt_mask: np.ndarray, pred_mask: np.ndarray) -> float:
	...
	----------
	gt_mask: ndarray
		Ground truth mask for a single object.
	pred_mask: ndarray
		Predicted mask for a single object.
	threshold: float
		Threshold value over which the prediction is deemed acceptable.

	Returns
	-------
	iou: float
		Intersection over Union raw value.
...
def compute_image_precision(gt_masks: list[np.ndarray], pred_masks: list[np.ndarray], thresholds: list[float]) -> tuple[float, list[float], list[tuple[float, float, float, float]]]:
	"""
	...
	thresholds: list[float]
		List of threshold values in [0,1].

	Returns
	-------
	precision: float
		Average precision over all thresholds.
	precision_per_threshold: list[float]
		List of individual precisions for each threshold in **thresholds**.
	tp_fp_fn_tn_per_threshold: list[tuple[float, float, float, float]]
		List of True Positive, False Positive, False Negative and True Negative for each threshold in **threshold**.
...
def evaluate_image(gt_dir: str, pred_dir: str, image_name: str, thresholds: list, output_dir: str) -> dict:
	...
	----------
	gt_dir: str
		Path to the directory containing ground truth object images.
	pred_dir: str
		Path to the directory containing predicted object images.
	image_name: str
		Name of the image without the extension.
	thresholds: list
		List of individual thresholds used to compute the intersection over union
	output_dir: str
		Path to the directory for outputting gt and pred visualisation images.

	Returns
	-------
	data: dict
		Path to the directory containing average precision and individual precisions for the evaluated image.
...
def export_stats(path: str, data: dict, thresholds: list[float]) -> None:
	"""
	...
	data: dict
		Data obtained from *evaluate_all_images()*.
	thresholds: list[float]
		List of thresholds used to compute precisions.
