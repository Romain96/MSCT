...
def generate_mser_tree_rec(
	...
	----------
	node: MultiScaleComponentTreeNode
		The node of the MSCT to process.
	parent: MSERNode
		The parent of the current branch in the MSERTree.
	tree: MSERTree
		The MSERTree to build.
	minima: set[MultiScaleComponentTreeNode]:
		The list of branch MSER minima.
	root: bool
		Whether the branch is the root branch of the MSCT/MSER tree.
...
def compute_mser_tree(root: MultiScaleComponentTreeNode) -> MSERTree:
	...
	-------
	mser_tree: MSERTree
		The computed MSERTree representing the given subtree.
...
def msct_nodes_from_filtered_msertree(mser_nodes_filtered: set[MSERNode]) -> set[MultiScaleComponentTreeNode]:
	"""
	Filters the MSCT nodes from the filtered MSERTree.

	Parameters
	----------
	mser_nodes_filtered: set[MSERNode]
		The set of MSER nodes to filter.

	Returns
	-------
	msct_nodes_filtered: set[MultiScaleComponentTreeNode]
		The set of filtered MSCT nodes.
...
def filter_mser_tree(tree: MSERTree, max_mser: float) -> set[MultiScaleComponentTreeNode]:
	"""
	Filters the MSER tree.

	Parameters
	----------
	tree: MSERTree
		The MSER tree to filter.
	max_mser: float
		The maximum MSER stability value allowed for a node to be retained.

	Returns
	-------
	filtered: set[MultiScaleComponentTreeNode]
		The set of filtered MSCT nodes.
...
def fill_holes(image: np.ndarray) -> np.ndarray:
	...
	----------
	image: ndarray
		A binary image (0, 255).

	Returns
	-------
	filled: ndarray
		The binary image (0, 255) with filled holes.
...
def smooth_and_threshold(image: np.ndarray) -> np.ndarray:
	...
	-------
	thresholded: ndarray
		The input image smoothed and thresholded.
...
def extract_connected_components(image: np.ndarray) -> list[np.ndarray]:
	...
	----------
	image: ndarray
		A grascale input image.

	Returns
	-------
	ccs: list[ndarray]
		The list of images, one per coonected component of the input image.
...
def perform_ultimate_erosion_and_partition(
	...
	----------
	inter_cluster: MultiScaleComponentTreeNode
		The MultiScaleComponentTreeNode to process.
	msct: MultiScaleComponentTree
		The MSCT from which objects belong to.
	image: ndarray
		The original image represented by the MSCT.
	min_area: int
		The minimum surface area of clusters.

	Returns
	-------
	all_objects: list[ndarray]
...
def ultimate_erosion(fz_img: np.ndarray, connectivity=[[0,1,0],[1,1,1],[0,1,0]]) -> list[np.ndarray]:
	...
	----------
	fz_img: ndarray
		The numpy array of the flat zone.
	connectivity : ndarray
		The connectivity to use for the ultimate erosion computation.

	Returns
	-------
	ccs: list[ndarray]
		The list of individual connected component as a list of images.
...
def find_object_centroids(ccs: list[np.ndarray]) -> list[tuple[int, int]]:
	...
	----------
	ccs: list[ndarray]
		The list of images representing each one connected component (non-zero values).
		
	Returns
	-------
	centroids: list[tuple[int, int]]
		The set of centroids (row, column).
...
def filter_centroids(
	...
	centroids_4: list[tuple[int, int]]
		The list of centroids of clusters obtained with ultimate erosion with 4-neighbourhood (CC).
	centroids_8: list[tuple[int, int]]
		The list of centroids of clusters obtained with ultimate erosion with 8-neighbourhood (CC).

	Returns
	-------
	centroids: list[tuple[int, int]]
		The list of centroids of final clusters to feed to the watershed algorithm.
...
def partitionize_data_watershed(
	...
	"""
	Partitionizes the given flat-zone of **node** using a watershed algorithm and the computed
	...
	distance: ndarray
		The distance transform.
	fz: ndarray
		The flat-zone of the node (modified).
	centroids: list[tuple[int, int]]
		The list of pixels (row, col) computed as centroids of segmentation classes.
	min_area: int
		The minimum surface area of connected components (classes).

	Returns
	-------
	list_of_classes: list[ndarray]
		The list of connected components as images.
...
def filter_watershed(big_ccs: list[np.ndarray], small_ccs: list[np.ndarray], image: np.ndarray, min_area: int) -> list[np.ndarray]:
	...
	Parameters
	----------
	big_ccs: list[ndarray]
		The list of big (> min_area) connected components (list of images).
	small_ccs: list[ndarray]
		The list of small (<= min_area) connected components (list of images).
	image: ndarray
		The flat zone image.
	min_area: int
		The minimum surface area in pixel for a cluster.

	Returns
	-------
	ccs: list[ndarray]
		The filtered and merged list of connected components (list of images).
...
def segmentation_define_intermediate_clusters(
	...
	"""
	Defines intermediate clusters by computing and filtering the MSER tree of the MSCT.

	Parameters
	----------
	candidate: MultiScaleComponentTreeNode
		The candidate node of the MSCT.
	max_mser: float
		The maximum MSER value allowed.

	Returns
	-------
	msct_nodes_filtered: list[MultiScaleComponentTreeNode]
		The list of intermediate clusters as a list of MSER tree nodes.
...
def segmentation_define_final_clusters(
	...
	"""
	Defines the final clusters by performing ultimate erosion.

	Parameters
	----------
	node: MultiScaleComponentTreeNode
		The MSCT node to process (local root).
	msct: MultiScaleComponentTree
		The MSCT.
	image: ndarray
		The image on which the MSCT is built.
	min_area: int
		The minimum surface area allowed (unused).

	Returns
	-------
	final_clusters: list[ndarray]
		The list of final clusters as a list of images.
...
def identify_single_objects(
	...
	----------
	msct: MultiScaleComponentTree
		The global MSCT to process.
	scale: int
		The scale at which to perform computation (default=0, i.e. the highest).
	image: ndarray
		The original image on which the MSCT is built.
	max_mser: float
		The Minimum MSER value for objects.
	min_area: int
		The minimum surface area of clusters.
	...
	objects: list[ndarray]
		The list of single objects (boolean images).
