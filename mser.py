...
class MSER:
	"""
	This class provides methods to compute **MSER** or *Maximally Stable Extremal Regions*.
	...
	def compute_mser_local_minima_per_branch(
		...
		----------
		node: MultiScaleComponentTreeNode
			A node of the MSCT to process.
		minima: set[MultiScaleComponentTreeNode]
			Set of all MSER local minima (nodes of the MSCT) per branch to fill.
		local_minima: MultiScaleComponentTreeNode
			Current node being the local minima of the current branch.
		local_minima_mser: float
			MSER value of **local_minima**.
	...
	def compute_mser_local_minima_per_branch_at_scale(
		...
		The local minima is the node along a branch having the higest stability 
		(i.e. the lowest `stability` attribute value).
		Only considers nodes at the given **scale**, not every branch might have a local minima !
		...
		node: MultiScaleComponentTreeNode
			A node of the MSCT to process.
		scale: int
			Scale at which the MSER nodes will be considered valid candidates.
		minima: set[MultiScaleComponentTreeNode]
			Set of all MSER local minima (nodes of the MSCT) per branch to fill.
		local_minima: MultiScaleComponentTreeNode
			Current node being the local minima of the current branch.
...
#------------------------------------------------------------------------------
	...
	@staticmethod
	def compute_mser_candidates(
		msct: MultiScaleComponentTree, 
		scale: int, max_area: int, max_mser: float
		...
		msct: MultiScaleComponentTree
			MSCT to process.
		scale: int
			Scale at which MSER nodes are considered valid (0 = original scale).
		max_area: int
			Maximum area allowed in true pixels (at original scale).
		max_mser: float
			Maximum MSER value allowed.

		Returns
		-------
		expanded_msers: list
			List of MSCT nodes to be selected for enrichment.
...
#------------------------------------------------------------------------------
	...
	def compute_mser_candidates_subtree(root: MultiScaleComponentTreeNode, scale: float, max_mser: float) -> list:
		...
		Returns
		-------
		expanded_msers: list[MultiScaleComponentTreeNode]
			List of MSCT nodes to be selected for enrichment.
...
#------------------------------------------------------------------------------
	...
	def msct_is_ancestor(node_a: MultiScaleComponentTreeNode, node_b: MultiScaleComponentTreeNode) -> bool:
		...
		-------
		: bool
			Returns *True* if **node_a** is an ancestor of **node_b**, *False* otherwise.
...
#------------------------------------------------------------------------------
	...
	def msct_is_descendant(node_a: MultiScaleComponentTreeNode, node_b: MultiScaleComponentTreeNode) -> bool:
		...
		-------
		: bool
			Returns *True* if **node_a** is a descendant of **node_b**, *False* otherwise.
...
#------------------------------------------------------------------------------
	...
	def remove_all_descendants(
		node: MultiScaleComponentTreeNode, 
		potential_descendants: list[MultiScaleComponentTreeNode]
	) -> list[MultiScaleComponentTreeNode]:
		"""
		...
		----------
		node: MultiScaleComponentTreeNode
			A node of the MSCT used as reference.
		potential_descendants: list[MultiScaleComponentTreeNode]
			A list of MSCT nodes to check.

		Returns
		-------
		non_descendants: list[MultiScaleComponentTreeNode]
			The sublist of MSCT nodes non descendants of **node** (subset of **potiential_descendants**).
...
#------------------------------------------------------------------------------
	...
	@staticmethod
	def remove_all_ancestors(
		node: MultiScaleComponentTreeNode, 
		potiential_ancestors: list[MultiScaleComponentTreeNode]
	) -> list[MultiScaleComponentTreeNode]:
		"""
		Removes all nodes of a list being descendants of a given node.
		...
		Returns
		-------
		non_ancestors: list[MultiScaleComponeneTreeNode]
			The sublist of MSCT nodes non ancestors of **node**, (subset of **potiential_ancestors**).
